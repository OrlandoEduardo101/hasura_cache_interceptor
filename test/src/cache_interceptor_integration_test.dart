...
void main() {
  ...
  group("no connection and", () {
    test(" no cache, throws exception", () async {
      ...
      when(() => httpClient.post(any(), body: any(named: 'body'), headers: any(named: 'headers'))).thenAnswer((realInvocation) async => mockResponse);
  ...
  group("have connection and", () {
    test("no cache, return real response", () async {
      ...
      when(() => httpClient.post(any(), body: any(named: 'body'), headers: any(named: 'headers'))).thenAnswer((realInvocation) async => http.Response(jsonEncode(realResponse), 200));
      when(() => storage.containsKey(any())).thenAnswer((realInvocation) async => false);
